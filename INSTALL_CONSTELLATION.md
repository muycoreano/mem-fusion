...
A persistent HTTP MCP daemon that federates group memory across multiple Mem-Fusion peers. Constellation runs alongside Mem-Fusion, sharing the same Qdrant instance but using its own canonical collection.

- **HTTP MCP daemon** on port 7533 (separate from Mem-Fusion's stdio MCP)
- **Canonical Qdrant collection** (`mem_fusion_canonical_memories`) — disjoint from Mem-Fusion's local memory
...
cat > ~/.local/share/mem-fusion/constellation/config.json <<'3079B827BF68_EOF'
{
  ...
  "canonical_collection": "mem_fusion_canonical_memories",
...
read -p "Drop the canonical Qdrant collection? [y/N] " yn
[[ "$yn" == "y" ]] && curl -X DELETE http://127.0.0.1:6333/collections/mem_fusion_canonical_memories

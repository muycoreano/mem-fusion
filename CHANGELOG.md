...
- **`docs/v0.5_SLACK_SYNCH.md`** — design doc for the Slack sync driver. Defines a peer transport addressable as `slack:#<channel>` where the channel itself is the peer group and manages membership. Covers wire format (human-readable message body + JSON file attachment), driver interface (push/pull/status/since), execution context (Claude-side, not daemon), notification design (Tier 1: Slack native UI; Tier 2: session-start auto-pull), Slack rate/size limits, human and machine performance considerations, and end-to-end validation plan. Implementation tracked under WP-E + WP-F. (WP-E/F design)

...
- **`docs/v0.5_SLACK_CONNECTOR.md`** — design doc for the Slack connector. Defines a memory-sync substrate declared as `{group_name, connector: "slack", channel}` where the channel itself is the peer group and manages membership. Introduces the **connector** terminology (replacing the working name "driver") and a uniform group config schema with two reserved fields (`group_name`, `connector`) plus connector-specific fields. `http` is the implicit default when `connector` is absent — v0.4 configs read as v0.5 valid without modification. Covers wire format (human-readable message body + JSON file attachment), connector interface (push/pull/status/since), execution context (Claude-side, not daemon), notification design (Tier 1 only for v0.5 — Slack's native channel UI; all Claude-routed notification mechanisms deferred to a future build), Slack rate/size limits, human and machine performance considerations, and end-to-end validation plan. Implementation tracked under WP-E (SlackConnector) + WP-F (Slack wire format). (WP-E/F design)
